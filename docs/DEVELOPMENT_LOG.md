...
### 补记：端到端实测 + 用真数据渲染界面时抓出的三个问题

**端到端实测（`test_desktop_e2e.py` 24/24，真跑本地 ASR，不花 AI 的钱）**
覆盖：探测（时长/有无音轨）→ 导入工作区 → 本地 ASR 出稿 → 项目落盘与读回（project_id 校验、状态机）→
界面绑定（顶栏/徽章/空态/按钮可用）→ 两项目互不污染 → 四种错误仍分层
（文件不存在=media_unreadable / 损坏=media_unreadable / 无音轨=no_audio_track / 静音=asr_empty）。

**抓出来的问题**
1. **五维评分条一根都不显示**：真实结果的 `dims` 键是**中文可读标签** `三秒吸引力（权重30%）`
   （`analysis/_entry_dims` 的产物），界面却按内部英文键 `hook/contrast/...` 去取 → 全落空。
   改成「结果里有哪些维度就画哪些」，并从键里解析权重一起显示。
2. **旧结果（D-041 之前没有 `recommended` 字段）文案误导**：原来会说「本场没有进入推荐名单的片段」，
   像是在说"AI 判定这场没价值"。改成如实说明「这份结果来自旧版本（还没有推荐标记）」。
3. **探测阶段不拦截无音轨视频**：原来要等用户填完设置、点开始跑，跑一段才失败。
   现在探测完就判定，无音轨 / 无法读取**立刻提示**。

**测试自身踩的坑（不是产品问题，但值得记）**
- 断言「切项目后旧内容没了」时用**行数**做指纹会误判（编号、时长里都可能出现同样的数字），
  改用**文字稿路径**做指纹才可靠。
...
## 2026-09-12  V0.5.2 桌面版加固（任务 1~6：进度 / 启动 / 自检 / 卸载 / 验收 / 报告）

**约束**：只做桌面软件工程问题。Story 架构、Event 聚合、AI 评分、推荐算法、UI 整体设计**一行未改**。

### 任务 1：真实处理进度
- 新增 `stages.py`（六阶段定义 + 结构化进度事件 + 带节流的上报器）、
  `desktop/ui/progress_view.py`（阶段路线图 + 真实百分比 + 细节）、
  `desktop/services/ai_progress.py`（把分析日志翻译成进度）
- **语音识别百分比是真的**：faster-whisper 每吐出一句话都带着这句话在音频里的结束时间，
  拿它当分子、视频总时长当分母 → `45%　23:10 / 51:00　已识别 312 句`
- 算不出进度的阶段（VAD / 音频提取）如实显示"进行中"，不编数字
- AI 阶段解析 `[海选] 区块 3/12`、`[复审] 批次 2/3` → 真百分比；其余里程碑 + 文字说明。**不改 analysis/**
- 进度信号从 `str` 改为结构化 dict（`desktop/workers.py`）
- 踩坑：`process_video` 会把传入的 `ProgressSink` 再包一层，内层把事件当"阶段名"，
  异常又被静默吞掉 → 表现为"进度一条都收不到"。改成幂等包装 + `make_event` 里对非字符串阶段做防御

### 任务 2：首次安装 / 启动不卡死
- 定位到的具体卡点：安装器在**主线程**解压 335 个条目 / 126 MB（`update_idletasks` 只重绘不处理事件）
  → Windows 标记「未响应」；`_dir_size_kb()` 递归遍历上万个文件；同步调 PowerShell 建快捷方式
- 修复：安装流程抽成模块级 `run_install()`（界面与静默模式共用），
  重活全部丢进后台线程，主线程每 60ms 轮询队列刷新；新增安装日志
- 启动：界面先显示 → 后台跑环境自检；项目列表改**异步加载**
- 踩坑：异步加载引入竞态 —— 异步读到的旧列表会覆盖之后的同步刷新。
  修法：加"有效期 token"，同步刷新时作废在途的异步结果
- 新增启动日志（含各阶段耗时）——实测启动到窗口显示 0.15~0.24 秒

### 任务 3：环境自检
- 新增 `desktop/services/selfcheck.py`：6 项检查 + **四类问题归类**（视频 / 程序组件 / 模型 / 权限）
- 单独查 **VAD 模型文件**（V0.5.1 就栽在这，所以它必须是一个独立检查项）
- `category_of_stage()` 把 ProcessError 的 stage 映射到四类；错误弹窗带类别前缀
- 启动后台自动跑一次，有问题才弹窗；「开发者视图 → 环境自检」可随时重查
- 顺带修：自检探针文件在失败路径会残留 → `finally` 里确保清理

### 任务 4：卸载修复
- **三个真因**（全部对上用户报的现象）：
  1. 卸载器自己就在安装目录里 → Windows 不允许删除运行中的程序文件 → 删目录必然失败
  2. PyInstaller 单文件程序的引导进程占着 `uninstall.exe`，实测退出要 **40 秒以上**，
     临时副本的重试窗口不够
  3. 删除失败被静默吞掉，注册表删除失败无人处理 →「应用和功能」一直残留
- 修复：卸载器先复制自己到 `%TEMP%` 再执行；`_rmtree_retry` 重试；
  删不掉时 `_schedule_late_cleanup` 交给系统后台继续删；**注册表必定清理**；全流程日志
- "程序还在运行"从**中止卸载**改为**警告但继续**（旧行为=点了卸载什么都没发生）
- **踩坑（重要）**：`subprocess.Popen(["cmd","/c", ...], creationflags=DETACHED_PROCESS)`
  **命令根本不会执行** —— cmd.exe 需要控制台。后台兜底清理因此静默失效，
  表现为"安排了清理但目录永远在"。改用带 BOM 的 PowerShell 脚本 + CREATE_NO_WINDOW 后，1 秒内删净
- 另一个坑：删除 `uninstall.exe` 失败时 `onerror` 把异常吞了，日志里连报错都没有 ——
  一度误判成"文件属性/权限问题"，实际是"等得不够久"

### 任务 5：安装卸载全流程验收
- 新增 `packaging/test_install_uninstall.py`（35 项）：静默安装 → 检查安装结果 → 首次启动 →
  真跑一次本地识别 → 静默卸载 → 逐个检查残留 → 重装并测试"连数据一起删"
- 为此给安装器加 `--silent / --dir=`、卸载器加 `--silent / --force / --purge=`
- 每次运行结束有兜底清理，不给测试机留垃圾

### 任务 6：开发报告
- `docs/V052_DESKTOP_HARDENING_REPORT.md`（修改文件清单 / 每个改动的原因 / 测试结果 /
  新增功能 / 剩余风险 / 教训固化）

### 测试结果
- `test_desktop_progress.py` **34/34**（含"百分比与已处理时间完全对应"这条硬验证）
- `packaging/test_install_uninstall.py` **35/35**
- `packaging/verify_install.py` **33/33**；`test_desktop_smoke.py` **52/52**；`test_desktop_e2e.py` **24/24**
- 回归 step1~7 = 23/21/24/17/11/24/20；UI 自测 15/15；ASR 分层 18/18
- 卸载实测：`卸载流程执行完毕（等了 1 秒）`、安装目录彻底删除、应用列表记录消失、
  桌面与开始菜单快捷方式清除、用户数据按预期保留/删除
