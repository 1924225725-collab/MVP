...
## v0.5.2（2026-09-12，桌面版加固：真实进度 / 启动体验 / 环境自检 / 卸载修复）

> 只做桌面软件工程问题。**Story 架构 / Event 聚合 / AI 评分 / 推荐算法 / UI 整体设计 一行未改。**

- **任务 1 · 真实处理进度**（新增 `stages.py`、`desktop/ui/progress_view.py`、`desktop/services/ai_progress.py`）
  - 六个阶段可视化：读取视频 → 提取音频 → VAD 人声检测 → 语音识别 → AI 分析 → 生成结果
  - **语音识别的百分比是真的**：已处理音频时间 / 视频总时长（如 `45%　23:10 / 51:00　已识别 312 句`），
    不是定时器刷出来的假动画
  - 算不出进度的阶段（VAD、音频提取）如实显示"进行中"，绝不编百分比
  - AI 分析阶段解析分析过程自身的日志（`[海选] 区块 3/12`、`[复审] 批次 2/3`）→ 海选/复审有真百分比，
    其余里程碑 + 明确写出"正在做什么"；**不改 analysis/ 一行代码**
  - 进度上报带节流，避免高频事件淹掉界面
- **任务 2 · 首次安装/启动不再卡死**
  - 定位到根因：安装器在**主线程**解压 12 万个文件 + 递归统计目录 + 同步调 PowerShell 建快捷方式 →
    Windows 标记「未响应」；启动时模型检测与项目列表也在主线程
  - 修复：安装器重活全部搬到后台线程（主线程每 60ms 轮询进度）；启动改为"界面先显示、自检后台跑"；
    项目列表改异步加载（并处理了异步结果覆盖新数据的竞态）
  - 新增**安装日志**（`%TEMP%\AILiveClipper_setup.log`，装完复制到安装目录）与
    **启动日志**（`logs/desktop_startup.log`，含每阶段耗时）—— 实测启动到窗口显示 0.15~0.24 秒
- **任务 3 · 环境自检系统**（新增 `desktop/services/selfcheck.py`）
  - 6 项检查：FFmpeg / 语音识别依赖 / **人声检测模型 VAD** / 本地模型 / 配置文件 / 数据目录权限
  - **四类问题归类**：视频问题 / 程序组件问题 / 模型问题 / 权限问题 —— 每类给不同解决办法
    （V0.5.1 的 VAD 漏文件属于"程序组件问题，请重新安装"，不再笼统说"推理失败"）
  - 启动后后台自动跑一次，有问题才弹窗；「开发者视图 → 环境自检」可随时查看与重新检查
  - 错误弹窗带类别前缀
- **任务 4 · 卸载程序修复**（重写 `packaging/uninstaller.py`）
  - 根因①：卸载器自己就在安装目录里 → 删不掉自己所在的目录；旧版只尝试一次，失败即永久残留
  - 根因②：PyInstaller 单文件程序的引导进程会占着 `uninstall.exe` 40 秒以上，临时副本等不到
  - 根因③：删除失败被静默吞掉、注册表删除失败无人处理 →「应用和功能」一直残留
  - 修复：卸载器先复制自己到 %TEMP% 再执行；删除带重试；删不掉的交给系统**后台继续删**
    （PowerShell 循环，实测 1 秒内删净）；**注册表必定清理**；全流程写日志
  - "程序还在运行"从**中止卸载**改为**警告但继续清理**（旧行为会导致点了卸载什么都没发生）
  - 踩坑：用 `DETACHED_PROCESS` 启动 `cmd.exe` 时命令**根本不执行**，后台清理静默失效 → 改用 PowerShell
- **任务 5 · 安装卸载全流程验收**（新增 `packaging/test_install_uninstall.py`，35 项）
  - 静默安装 → 检查安装结果 → 首次启动 → 真跑一次本地识别 → 静默卸载 → 检查残留 → 重装并测试删数据分支
- **任务 6 · 开发报告**：`docs/V052_DESKTOP_HARDENING_REPORT.md`
- **新增命令行能力**（供自动化/运维）
  - `AILiveClipper_Setup.exe --silent --dir=D:\xxx [--no-desktop] [--no-startmenu]` / `--selftest`
  - `uninstall.exe --silent --dir=D:\xxx [--purge=1] [--force]`
- **测试结果**
  - `test_desktop_progress.py` **34/34**（含"百分比与已处理时间完全对应"这条硬验证）
  - `packaging/test_install_uninstall.py` **35/35**（安装/启动/识别/卸载全绿）
...
- **顺带修掉**：自检的临时探针文件在失败路径会残留（`finally` 里确保清理）
